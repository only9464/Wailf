...
Wailf 是面向授权安全工作的本地优先桌面平台，覆盖资产侦察、指纹识别、漏洞验证、会话管理、取证响应和威胁情报等能力域。项目使用 Wails 3 + Go + Vue 3；业务服务仍在接入阶段，前端默认明确显示“服务尚未接入”。
...
- [前端架构](docs/architecture/frontend.md)：工作台、路由、状态、组件库和浮层。
- [前端开发规范](docs/development/frontend-conventions.md)：目录、单语言 JSON、Element Plus/shadcn-vue/Vue Bits 规则。
- [领域模型与持久化](docs/architecture/domain-model-and-storage.md)：应用级全局数据、TargetScope、Job、Asset 与 Artifact。
...
- [安全与治理](docs/security/governance.md)：授权、审计、秘密和保留策略。
...
| 数据模型 | 本机单用户、应用级全局业务数据；独立 TargetScope 约束主动操作 |
...
全局任务、资产、产物和会话可直接查询；主动扫描等操作仍需明确选择有效 TargetScope。撤销 Scope 不删除历史任务、产物或审计。业务草稿、查询快照和界面通知不写入 localStorage。Wailf 只面向获得授权的安全工作，不提供未授权入侵、绕过检测或载荷投递步骤。
