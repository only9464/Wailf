...
{
  "code": "input.invalid",
  "messageKey": "error.input.invalid",
  "detail": {"field": "targets"},
...
列表通过筛选、分页和排序访问全局数据；详情通过 `jobId`、`assetId`、`artifactId` 或 `sessionId` 查询。扫描提交包含目标、profile 和 connector，取消通过 `jobId` 发起并校验应用策略。策略变化不删除历史查询结果。

概念事件提示只携带定位信息，例如 `{ "jobId": "job_opaque_id" }`；GUI 收到后重新查询。切换任务或筛选后，旧响应不得覆盖当前状态。错误和事件都不增加项目标识字段。
...
首期 CLI 暴露候选子集：资产查询与导出、端口扫描 Job、Job 查询/取消和 Artifact 导出。交互式 Session 控制不作为首期默认命令。
...
工具输入按操作声明目标和输出大小限制；危险动作默认拒绝；MCP 不默认暴露原始 C2/WebShell 命令执行。
...
在这些条件未完成前，server 模式只能作为开发或受控本地部署，不宣传为远程控制面。
