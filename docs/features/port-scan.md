...
1. 应用级策略允许主动扫描；
2. TargetScope 有明确授权说明和有效期；
3. 请求输入能归一化为允许的目标表达式；
4. Connector 已配置、健康检查通过并声明版本；
5. Scope 生效、未过期、未撤销，并允许当前操作和目标；
6. 高风险范围或并发设置已经得到用户明确确认。

任一条件不满足时，不创建运行中的 Job；可以创建带拒绝原因的审计记录，便于用户修正配置。
...
sequenceDiagram
    ...
    participant Scope as TargetScope Policy
...
领域服务负责授权、策略、Job 和归一化；Connector 负责与具体工具或协议交互。更换外部工具不应改变 Asset、Job 和 Artifact 的公共字段。

## 6. 三入口示例

### GUI

GUI 页面分为全局授权范围选择与管理、目标输入、profile/Connector 选择、授权确认、运行状态和任务关联结果。全局任务查询无需先选 Scope，Asset 不按 Scope 分区；观测和任务保留授权来源。启动后立即显示 Job 摘要，TaskPanel 可以在不离开当前页面的情况下查看进度和取消。

当前仅实现前端流程，默认服务返回未接入；用户可编辑本次运行的目标与 Scope 草稿，保存、提交、取消、业务导出不可用并说明原因。Connector/profile 不生成演示选项。选择 Scope 不清空目标草稿，但使旧提交确认失效。
...
```text
wailf cli recon portscan --scope scope_opaque_id --input targets.txt --output json
```

stdout 返回 Job JSON，stderr 只输出诊断；用户使用 `job get` 或 `job wait` 获取完成结果。命令不会读取 GUI 布局，也不会绕过 Scope 校验。

### MCP

使用聚合工具提交经过确认的扫描任务，返回 Job 摘要和下一步查询建议。工具描述必须限制输入大小、目标范围和输出体积；MCP 默认不能直接执行任意外部命令。

## 7. 失败与恢复

| 场景 | Job 结果 | 处理 |
| --- | --- | --- |
| 提交前 Scope 过期/撤销 | 不创建 Job，返回授权错误 | 写拒绝审计，提示更新范围 |
| 运行期间 Scope 失效 | 按策略停止，状态为 `failed` | 保存已获结果，附稳定错误 code，不删除历史 |
